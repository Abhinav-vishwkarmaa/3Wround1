...
function App() {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get("https://threew1.onrender.com/users");
      setUsers(res.data);
    } catch (err) {
  ...
  const claimPoints = async (userId) => {
    try {
      await axios.post(`https://threew1.onrender.com/claim/${userId}`);
      fetchUsers();
    } catch (err) {
  ...
  const deleteUser = async (userId) => {
    try {
      await axios.delete(`https://threew1.onrender.com/users/${userId}`);
      fetchUsers();
    } catch (err) {
